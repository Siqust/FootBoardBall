...
namespace Mirror.Examples.MultipleMatch
{
    public class CanvasController : MonoBehaviour
    {
        ...
        public TMP_InputField matchCodeInputField;
        ...
        {
            NetworkClient.Send(new ServerMatchMessage { serverMatchOperation = ServerMatchOperation.CreatePrivate });
        }
        ...
        public void RequestJoinMatch()
        {
            ...
            {
                var match = openMatches[child.GetComponent<MatchGUI>().GetMatchId()];
                if (match.players < match.maxPlayers && match.matchCode=="")
                {
                    NetworkClient.Send(new ServerMatchMessage { serverMatchOperation = ServerMatchOperation.Join, matchId = match.matchId });
                    return;
                }
            }
            NetworkClient.Send(new ServerMatchMessage { serverMatchOperation = ServerMatchOperation.Create });
        }
        public void RequestJoinPrivateMatch()
        {
            string code = matchCodeInputField.text;
            foreach (Transform child in matchList.transform)
            {
                var match = openMatches[child.GetComponent<MatchGUI>().GetMatchId()];
                if (match.players < match.maxPlayers && match.matchCode == code)
                {
                    NetworkClient.Send(new ServerMatchMessage { serverMatchOperation = ServerMatchOperation.Join, matchId = match.matchId });
                    return;
                }
            ...
            Invoke("disable_notification_not_found", 3f);
        }
        private void disable_notification_not_found()
        {
        ...
        void OnServerMatchMessage(NetworkConnectionToClient conn, ServerMatchMessage msg)
        {
            ...
            switch (msg.serverMatchOperation)
            {
                ...
                case ServerMatchOperation.CreatePrivate:
                    {
                        OnServerCreatePrivateMatch(conn);
                        break;
                    }
        ...
        void OnServerCreateMatch(NetworkConnectionToClient conn)
        {
            ...
            openMatches.Add(newMatchId, new MatchInfo { matchId = newMatchId, maxPlayers = 2, players = 1, matchCode = "" });
        ...
        void OnServerCreatePrivateMatch(NetworkConnectionToClient conn)
        {
            if (playerMatches.ContainsKey(conn)) return;
            ...
            while (true)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    code += st[UnityEngine.Random.Range(0, st.Length)];
                }
                foreach (MatchInfo match in openMatches.Values)
                {
                    if (match.matchCode == code)
                    {
                        free = false;
                    }
                }
